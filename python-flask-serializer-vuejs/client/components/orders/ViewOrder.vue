    ...
    <v-flex xs12 sm12>
        ...
        <v-container fluid grid-list-xl>
          <v-layout wrap align-center>
            <v-flex xs12 sm4 d-flex>
              <v-text-field readonly v-model="order.factory" label="Fábrica"></v-text-field>
            </v-flex>
            ...
            <v-flex xs12 sm8 d-flex>
              <v-menu ref="menu" :close-on-content-click="true" v-model="menu" :nudge-right="40" :return-value.sync="date" lazy transition="scale-transition" offset-y full-width min-width="290px">
                <v-text-field slot="activator" v-model="order.receivedDate" label="Data de Recebimento" prepend-icon="event"></v-text-field>
                <v-date-picker v-model="order.receivedDate" no-title scrollable>
                  <v-spacer></v-spacer>
      ...
      <v-dialog v-model="editDialog" max-width="500px">
        <v-card>
              ...
              <v-layout wrap>
                <v-flex xs12 sm12>
                  <v-text-field v-model="choosedItem.qty" 
                                ...
                                required
                                :rules="[() => choosedItem.qty > 0 || 'Quantidade Inválida']"></v-text-field>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-text>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken-1" flat @click.native="saveEditedItem" :disabled="!isValidQty"> OK </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>

      <!-- DIALOGO CONFIRMAÇÃO PEDIDO -->
      <v-dialog v-model="confirmOrderDialog" width="500">
        <v-btn v-if="order.status !== 'CONFIRMADO'" slot="activator" outline @click="confirmOrder"> Confirmar Recebimento </v-btn>

        <v-card>
          <v-card-text>
            O recebimendo do pedido foi confirmado!
          </v-card-text>
...
<script>
  ...
  export default {
    ...
    async mounted () {
      ...
      let response = await OrdersService.get(orderId, this.$store.state.token)
    ...
    methods: {
      ...
      saveEditedItem () {
        let editedItem = this.order.items.find((obj) => { return obj.code === this.choosedItem.code })
        editedItem.qty = this.choosedItem.qty
      ...
      confirmOrder () {
        ...
        //OrdersService.confirm(order, this.$store.state.token)
      },
